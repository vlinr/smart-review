...
const MESSAGES = {
  'zh-CN': {
    ...
    interrupt_prompt: '等待输入命令 (q/esc 退出)：',
    usage_header: '使用方法:',
    ...
    skill_diff_risk_guard: '仅聚焦本次变更引入的风险，不重复陈述历史遗留问题。',
    skill_evidence_enforcer: '每个问题必须给出证据片段、风险原因、可执行修改建议。',
    skill_security_deep: '深度检查注入、鉴权、越权、敏感数据泄露、配置风险。',
    skill_logic_correctness: '检查分支条件、边界处理、空值路径、错误传播是否正确。',
    skill_api_contract: '检查接口输入输出契约、幂等性、状态码语义、兼容性。',
    skill_performance_hotpath: '检查热路径复杂度、批处理机会、重复计算与缓存策略。',
    skill_concurrency_resource: '检查并发竞态、取消传播、连接/句柄/定时器释放。',
    skill_maintainability_testability: '检查可维护性、可测试性、可观测性与模块边界。',
    skills_prompt_header: '[审查技能]',
    skills_prompt_mode: '模式: {mode}',
    skills_prompt_must_execute_all: '你必须在同一回复中执行所有列出的技能。',
    skills_prompt_output_header: '[输出要求]',
    skills_prompt_output_rule1: '每个问题必须包含：文件路径、具体代码片段、风险原因、可执行修改建议。',
    skills_prompt_output_rule2: '不要输出没有直接代码证据的泛化结论。',
    skills_prompt_target_files_header: '[目标文件]',
    skills_correction_line1: '你上一条回复未满足必需的审查技能约束。',
    skills_correction_line2: '请严格按以下技能重新执行: {skills}',
    skills_correction_line3: '只返回包含以下字段的问题：文件路径、具体代码片段、风险原因、可执行修改建议。',
    tools_prompt_header: '[本地只读工具]',
    tools_prompt_how_to_call: '如需调用工具，请且仅返回一个代码块：',
    tools_prompt_or: '或',
    tools_prompt_budget: '工具调用预算：最多 {maxCalls} 次。',
    tools_prompt_strategy1: '默认策略：优先使用已提供审查上下文，仅在关键证据缺失时调用工具。',
    tools_prompt_strategy2: '需要调用工具时：优先 list_changed_files/get_staged_diff，再按需 read_file/search。',
    tools_prompt_final_format: '最终输出必须包裹在 [REVIEW_FINAL]...[/REVIEW_FINAL] 中，并给出实质审查结果。',
    tools_prompt_completion_rule: '会话完成规则：返回实质代码审查结果（问题或“无/none”），而不是工具调用。',
    tools_prompt_no_tools_needed: '若无需工具，直接返回最终审查问题。',
    tools_finalize_line1: '工具调用预算已用尽或工具链已结束。',
    tools_finalize_line2: '不要再请求任何额外工具调用。',
    tools_finalize_line3: '现在请直接返回最终且实质的代码审查结果。',
    tools_finalize_line4: '最终输出请包裹在 [REVIEW_FINAL]...[/REVIEW_FINAL] 中。',
    tools_finalize_line5: '如果没有发现问题，请返回“无”或“none”。',
  ...
  'en-US': {
    ...
    interrupt_prompt: 'Waiting for command (q/esc to exit): ',
    usage_header: 'Usage:',
    ...
    skill_evidence_enforcer: 'Each issue must include evidence snippet, risk reason, and actionable fix suggestion.',
    ...
    skill_api_contract: 'Check API input/output contracts, idempotency, status semantics, and compatibility.',
    skill_performance_hotpath: 'Check hot-path complexity, batching opportunities, duplicate work, and caching.',
    ...
    skills_prompt_output_rule1: 'Every issue must include file path, concrete snippet, risk reason, and actionable suggestion.',
    ...
    tools_prompt_how_to_call: 'You may request tools by returning exactly one block:',
    tools_prompt_or: 'or',
    tools_prompt_budget: 'Tool call budget: up to {maxCalls} calls.',
    tools_prompt_strategy1: 'Default strategy: prefer provided review context first; call tools only when key evidence is missing.',
    tools_prompt_strategy2: 'When tools are needed: prefer list_changed_files/get_staged_diff first, then targeted read_file/search tools.',
    ...
    tools_finalize_line5: 'If no issue is found, return "none" or "无".',
...
export function buildPrompts(localeOrConfig) {
  const loc = getLocale(localeOrConfig);
  const L = FIELD_LABELS[loc] || FIELD_LABELS['en-US'];
