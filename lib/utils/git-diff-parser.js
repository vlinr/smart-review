...
import { SEGMENTATION_CONSTANTS } from './constants.js';
...
export class GitDiffParser {
  ...
  applySmartSegmentation(sections, filePath) {
    const MAX_TOKENS_PER_SEGMENT = SEGMENTATION_CONSTANTS.DEFAULT_MAX_TOKENS_PER_SEGMENT;
    const ESTIMATED_CHARS_PER_TOKEN = SEGMENTATION_CONSTANTS.DEFAULT_TOKEN_RATIO;
  ...
  /**
   * 获取指定文件集合的 git diff 审查数据（先暂存区，无变更则回退工作区 diff）
   */
  async getFilesDiffInspection(relativePaths = [], options = {}) {
    const paths = (relativePaths || []).map((p) => String(p || '').replace(/\\/g, '/')).filter(Boolean);
    if (paths.length === 0) return { reviewData: [], touchedPaths: [] };

    const staged = options.staged !== false;
    const fallbackUnstaged = options.fallbackUnstaged !== false;
    let diffOutput = await this.getDiffForPaths(paths, staged);

    if (!diffOutput.trim() && staged && fallbackUnstaged) {
      diffOutput = await this.getDiffForPaths(paths, false);
    }

    if (!diffOutput.trim()) {
      return { reviewData: [], touchedPaths: [] };
    }

    const files = this.parseDiffOutput(diffOutput);
    const touchedPaths = files.map((file) => String(file?.filePath || '').replace(/\\/g, '/')).filter(Boolean);
    const reviewData = [];
    for (const fileInfo of files) {
      ...
      if (reviewContent) {
        const fullContent = (await this.getStagedFileContent(fileInfo.filePath))
          || (await this.getWorkingFileContent(fileInfo.filePath));
        reviewData.push({
          ...reviewContent,
          fullContent,
          isDiffMode: true
  ...
  async getFilesDiffReviewData(relativePaths = [], options = {}) {
    const { reviewData } = await this.getFilesDiffInspection(relativePaths, options);
    return reviewData;
  }

  async getDiffForPaths(relativePaths, staged = true) {
    try {
      const args = staged
        ? ['diff', '--cached', `-U${Math.max(3, this.contextMergeLines || 10)}`, '--', ...relativePaths]
        : ['diff', `-U${Math.max(3, this.contextMergeLines || 10)}`, '--', ...relativePaths];
  ...
  async getWorkingFileContent(filePath) {
    try {
