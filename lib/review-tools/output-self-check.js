...
import { isIssueFieldLine, hasPollutedProseField } from '../utils/issue-field-normalize.js';
...
/**
 * True when parsed issue snippets / prose still contain report field lines.
 */
export function hasPollutedParsedSnippets(issues = []) {
  return (issues || []).some((issue) => {
    const snippet = String(issue?.snippet || '');
    if (snippet.trim() && snippet.split('\n').some((line) => isIssueFieldLine(line))) {
      return true;
    }
    if (hasPollutedProseField(issue?.message) || hasPollutedProseField(issue?.suggestion)) {
      return true;
    }
    return false;
  });
