...
export function extractFinalReviewContent(content) {
  ...
  if (closed.length > 0) {
    ...
    if (substantive.length > 0) {
      // Tools/finalize often emit multiple closed drafts; joining them duplicates
      // mid-issue fields into parse. Prefer the last complete draft only.
...
export function isSuccessfulReviewOutput(content, issueCount = 0, issues = []) {
  if (Number(issueCount) > 0) {
    const polluted = (issues || []).some((issue) => {
      ...
      if (snippet.trim() && /(?:^|\n)\s*(?:风险等级|Risk\s*Level|代码片段|Code\s*Snippet|行号|Line|问题\d+|Issue\s*\d+)\s*[:：]?/m.test(snippet)) {
        return true;
      }
      const prose = `${issue?.message || ''}\n${issue?.suggestion || ''}`;
      return /(?:^|\n)\s*(?:风险等级|Risk\s*Level|代码片段|Code\s*Snippet|行号|Line|问题\d+|Issue\s*\d+)\s*[:：]/m.test(prose);
    });
    return !polluted;
  }
  const text = extractFinalReviewContent(content);
