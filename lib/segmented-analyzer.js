...
import { SEGMENTATION_CONSTANTS } from './utils/constants.js';
import { t, getLocale, FIELD_LABELS, buildPrompts, displayRisk } from './utils/i18n.js';
...
export class SegmentedAnalyzer {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        try {
          ...
          if (this.config.enableSummaryContext && segmentResult.summary) {
            ...
            // 更新上下文（保留最近几段的摘要）— 喂给 AI，固定英文
            const recentSummaries = segmentSummaries.slice(-3); // 保留最近3段摘要
            const aiCfg = { locale: 'en-US' };
            previousContext = [t(aiCfg, 'segment_prev_context_header'),
              ...recentSummaries.map(s => t(aiCfg, 'segment_prev_context_item', { index: s.segmentIndex, range: s.lineRange, summary: s.summary }))
            ].join('\n');
          }
  ...
  async analyzeSegment(segment, filePath, aiClient, customPrompt, staticIssues, previousContext, currentSegmentNum, totalSegments) {
    try {
      ...
      const payload = {
        ...
        messages: [
          { role: 'system', content: this.getSegmentSystemPrompt(aiClient) },
          { role: 'user', content: segmentPrompt }
        ]
      };
      const maxTokens = aiClient.getMaxTokens ? aiClient.getMaxTokens() : undefined;
      if (maxTokens) payload.max_tokens = maxTokens;
      const response = await aiClient.chatWithRetry(payload);
  ...
  buildSegmentPrompt(segment, filePath, cleanContent, customPrompt, staticIssues, previousContext, currentSegmentNum, totalSegments) {
    ...
    let prompt = t(aiCfg, 'segment_prompt_template', {
    ...
    if (staticIssues && staticIssues.length > 0) {
      ...
      if (relevantStaticIssues.length > 0) {
        prompt += `\n\n${t(aiCfg, 'segment_static_issues_header', { index: currentSegmentNum })}\n`;
        relevantStaticIssues.forEach((issue, idx) => {
          const riskDisp = displayRisk(issue.risk || 'suggestion', aiCfg);
          const snippetLabel = L.snippet;
          const suggest = issue.suggestion ? ` ${issue.suggestion}` : '';
          prompt += t(aiCfg, 'segment_static_issue_line', {
            index: idx + 1,
            risk: riskDisp,
  ...
  /**
   * 获取分段分析的系统提示词（与主路径一致：内部英文 + 用户输出语言要求）
   */
  getSegmentSystemPrompt(aiClient) {
    if (aiClient && typeof aiClient.getSystemPrompt === 'function') {
      return aiClient.getSystemPrompt();
    }
    const { systemPrompt } = buildPrompts({ locale: 'en-US' });
    return systemPrompt;
  }
