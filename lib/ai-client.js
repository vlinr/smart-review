...
import { countCompleteIssueBlocks } from './review-tools/review-stability.js';
import {
...
export function mapRiskLevel(levelText) {
  ...
  // Prefer explicit severity tokens before soft words like "recommendation",
  // which often appear as annotations on High/Medium findings.
  if (/致命|critical|blocker|fatal/.test(compact) || /\bcritical\b/.test(raw)) return 'critical';
  if (/高危|severe|major/.test(compact) || compact === 'high' || compact === '高' || /\bhigh\b/.test(raw)) {
    return 'high';
  }
  if (/中危|medium|moderate|中等/.test(compact) || compact === '中' || /\bmedium\b/.test(raw)) {
    return 'medium';
  }
  if (/低危|minor/.test(compact) || compact === 'low' || compact === '低' || /\blow\b/.test(raw)) {
    return 'low';
  }
  if (/建议|suggestion|info|tip|advice|recommendation/.test(compact) || /\bsuggestion\b/.test(raw)) {
    return 'suggestion';
  }
  return 'suggestion';
}
...
/**
 * True only for user interrupt / AbortController / SDK abort — not arbitrary
 * provider messages that happen to contain "cancel" or "abort".
 */
export function isCancellationError(error) {
  return matchCancellationError(error);
}

function matchCancellationError(error) {
  ...
  if (/(?:^|\b)(?:the |this )?operation was aborted\b/i.test(msg)) return true;
  if (/(?:^|\b)request was aborted\b/i.test(msg)) return true;
  if (/审查已被中断/.test(msg)) return true;
  if (/^review interrupted\.?$/i.test(msg)) return true;
  if (lower === 'aborted' || lower === 'canceled' || lower === 'cancelled') return true;
  return false;
}
...
export class AIClient {
    ...
    while (true) {
      ...
      try {
        ...
        if (meta && meta.skillContext && meta.skillContext.strict) {
          const originalRes = finalRes;
          const originalMessages = finalMessages;
          const originalContent = finalRes?.choices?.[0]?.message?.content || '';
          let content = originalContent;
          const maxStrictAttempts = 3;
          let strictAttempt = 0;
          while (
            content.trim()
            && !this.validateSkillResponse(content, meta.skillContext)
            && strictAttempt < maxStrictAttempts
          ) {
            strictAttempt += 1;
            logger.progress(t(this.config, 'ai_progress_skill_strict', {
              file: this.getProgressFileLabel(),
              attempt: strictAttempt,
              max: maxStrictAttempts
            }));
            ...
            const nextContent = strictRes?.choices?.[0]?.message?.content || '';
          ...
          if (content.trim() && !this.validateSkillResponse(content, meta.skillContext)) {
            const originalExtracted = this.extractFinalReviewContent(originalContent);
            const lastExtracted = this.extractFinalReviewContent(content);
            const originalCount = countCompleteIssueBlocks(originalExtracted);
            const lastCount = countCompleteIssueBlocks(lastExtracted);
            if (originalCount > 0 && originalCount >= lastCount) {
              finalRes = originalRes;
              finalMessages = originalMessages;
              logger.progress(t(this.config, 'ai_progress_skill_strict_keep_original', {
                file: this.getProgressFileLabel(),
                count: originalCount
              }));
            }
  ...
  async resolveToolCalls(params, initialResponse, signal, toolsCfg) {
    ...
    for (let i = 0; i < maxCalls; i++) {
      ...
      logger.progress(t(this.config, 'ai_progress_tool_round_done', {
        file: this.getProgressFileLabel(),
        used: callsUsed,
        max: maxCalls
      }));
  ...
  async createChatCompletion(params, signal) {
    ...
    const stopHeartbeat = this.startWaitHeartbeat(startedAt);
    let response;
    try {
  ...
  /**
   * Emit periodic "still waiting" lines while a model round is in flight.
   * Interval grows gently with light jitter so it feels alive, not robotic.
   */
  startWaitHeartbeat(startedAt) {
    let tick = 0;
    let timer = null;
    let stopped = false;
    const nextDelayMs = () => {
      // Soft ramp: ~7–12s, then ~10–18s, then ~14–25s, capped around half a minute.
      const base = Math.min(7000 + tick * 3500, 22000);
      const spread = Math.min(5000 + tick * 1500, 10000);
      const jitter = Math.floor(Math.random() * (spread + 1)) - Math.floor(spread / 2);
      return Math.max(5000, Math.min(30000, base + jitter));
    };
    const schedule = () => {
      if (stopped) return;
      ...
      tick += 1;
      timer = setTimeout(() => {
        if (stopped || this.isCancelled()) return;
        const secs = Math.max(1, Math.round((Date.now() - startedAt) / 1000));
        logger.progress(t(this.config, 'ai_progress_still_waiting', {
          file: this.getProgressFileLabel(),
          secs
        }));
        schedule();
  ...
  parseIssueBlock(block, filePath, context = {}) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (collectReason) {
        if (isFieldLine(line)) {
          issue.message = sanitizeIssueProseField(reasonLines.join('\n'));
          collectReason = false;
          reasonLines = [];
        } else {
      ...
      if (collectSuggestion) {
        if (isFieldLine(line)) {
          issue.suggestion = sanitizeIssueProseField(suggestionLines.join('\n'));
      ...
      if (field.kind === 'file') {
        ...
        collectReason = true;
        reasonLines = field.value ? [field.value] : [];
    ...
    if (collectSuggestion && (!issue.suggestion || issue.suggestion.length === 0)) {
      ...
      issue.suggestion = sanitizeIssueProseField(issue.suggestion);
    }
    if (collectFixSnippet && (!issue.fixSnippet || issue.fixSnippet.length === 0)) {
      issue.fixSnippet = fixSnippetLines.join('\n').trim();
    }
    if (issue.message) {
      issue.message = sanitizeIssueProseField(issue.message);
    }
