...
export class AIClient {
  ...
  maskSensitiveText(text = '') {
    return String(text)
      .replace(/(api[_-]?key["'\s:=]+)([A-Za-z0-9._\-]+)/ig, '$1***')
      .replace(/(authorization["'\s:=]+bearer\s+)([A-Za-z0-9._\-]+)/ig, '$1***')
      .replace(/([?&]key=)([^&\s]+)/ig, '$1***');
  }
  ...
  async analyzeSmartBatch(batchData, originalBatch = null, requestMeta = null) {
    try {
      ...
      const batchIntro = this.tForAI('batch_intro');
      const messages = [
      ...
      const smartToolPrompt = this.buildToolsPrompt();
      if (smartToolPrompt) {
        messages.push({ role: 'user', content: smartToolPrompt });
      }
      // 处理每个文件
      ...
      if (includeStaticHints) {
        ...
        for (const file of batchData.files) {
          ...
          const lines = staticIssues.map((i, idx) => {
            const riskDisp = displayRisk(i.risk || 'suggestion', this.aiLocaleConfig);
            const suggestPart = i.suggestion ? this.tForAI('inline_suggestion', { suggestion: i.suggestion }) : '';
          ...
          const header = this.tForAI('local_rule_findings_header', { file: file.filePath });
      ...
      const finalInstructionBatch = this.tForAI('final_instruction_batch');
      messages.push({ role: 'user', content: finalInstructionBatch });
      // 追加严格忽略规则，避免模型输出“行号跳跃/预处理移除”的提示
      const finalIgnoreRule = this.tForAI('ignore_rule');
    ...
    try {
      ...
      const analyzeOne = async (i) => {
        ...
        try {
          ...
          const segmentToolPrompt = this.buildToolsPrompt();
          if (segmentToolPrompt) {
            messages.push({ role: 'user', content: segmentToolPrompt });
          }
          ...
          if (includeStaticHints && file.staticIssues && file.staticIssues.length > 0) {
            ...
            if (segmentStaticIssues.length > 0) {
              const lines = segmentStaticIssues.map((si, idx) => {
                const riskDisp = displayRisk(si.risk || 'suggestion', this.aiLocaleConfig);
                const suggestPart = si.suggestion ? this.tForAI('inline_suggestion', { suggestion: si.suggestion }) : '';
                return this.tForAI('segment_static_issue_line', {
                  index: idx + 1,
                  risk: riskDisp,
                  message: si.message,
                  suggest: suggestPart,
              ...
              messages.push({ role: 'user', content: `${this.tForAI('segment_static_issues_header', { index: i + 1 })}\n${lines.join('\n')}` });
            }
          }
  ...
  async analyzeDiffFile(fileData, options = {}) {
    try {
      ...
      const diffToolPrompt = this.buildToolsPrompt();
      if (diffToolPrompt) {
        messages.push({ role: 'user', content: diffToolPrompt });
      }

      // 构建diff分析提示
      const intro = this.tForAI('diff_intro');
      ...
      for (let i = 0; i < fileData.segments.length; i++) {
        ...
        const segmentPrompt = `\n[${segTitle}] (${segMeta})\n\`\`\`diff\n${segment.content}\n\`\`\``;
          ...
          // 追加严格忽略规则，避免模型输出“行号跳跃/预处理移除”的提示
          messages.push({ role: 'user', content: this.tForAI('ignore_rule') });
      }
      ...
      // 添加最终指令
      const finalInstruction = this.tForAI('diff_final_instruction', { file: fileData.filePath });
      ...
      // 发送请求并处理响应
      const diffReqId = this.createRequestId('diff', fileData.filePath);
  ...
  async analyzeFilesBatch(entries) {
    try {
      ...
      const messages = [
        ...
        {
          role: 'user',
          content: this.tForAI('batch_files_intro')
        }
      ];
      ...
      const batchToolPrompt = this.buildToolsPrompt();
      if (batchToolPrompt) {
        messages.push({ role: 'user', content: batchToolPrompt });
      }
      ...
      if (includeStaticHints) {
        ...
        for (const e of entries) {
          ...
          const header = this.tForAI('local_rule_findings_header', { file: e.filePath });
          hintsParts.push(`${header}\n${lines.join('\n')}`);
        }
      if (hintsParts.length > 0) {
          messages.push({ role: 'user', content: hintsParts.join('\n\n') });
  ...
  getReviewSkillCatalog() {
    return {
      DIFF_RISK_GUARD: this.tForAI('skill_diff_risk_guard'),
      EVIDENCE_ENFORCER: this.tForAI('skill_evidence_enforcer'),
      SECURITY_DEEP: this.tForAI('skill_security_deep'),
      LOGIC_CORRECTNESS: this.tForAI('skill_logic_correctness'),
      API_CONTRACT: this.tForAI('skill_api_contract'),
      PERFORMANCE_HOTPATH: this.tForAI('skill_performance_hotpath'),
      CONCURRENCY_RESOURCE: this.tForAI('skill_concurrency_resource'),
      MAINTAINABILITY_TESTABILITY: this.tForAI('skill_maintainability_testability')
    };
  }
  ...
  getSkillsConfig() {
    ...
    return {
      ...
      routes: Array.isArray(skills.routes) ? skills.routes : [],
      modeDefaults
    };
  }
  ...
  resolveSkillSet(mode, filePaths = []) {
    ...
    const routeSkills = this.resolveRouteSkills(mode, filePaths, cfg.routes);
    const merged = this.normalizeSkillIds([...cfg.required, ...defaults, ...cfg.optional, ...routeSkills]);
  ...
  resolveRouteSkills(mode, filePaths = [], routes = []) {
    if (!Array.isArray(routes) || routes.length === 0) return [];
    const normalizedMode = String(mode || '').toLowerCase();
    const normalizedPaths = (Array.isArray(filePaths) ? filePaths : []).map((p) => String(p || '').replace(/\\/g, '/'));
    const skills = [];
    for (const route of routes) {
      if (!route || typeof route !== 'object') continue;
      const add = this.normalizeSkillIds(route.add || []);
      if (add.length === 0) continue;
      const modes = Array.isArray(route.modes) ? route.modes.map((m) => String(m).toLowerCase()) : [];
      if (modes.length > 0 && !modes.includes(normalizedMode)) continue;
      const patterns = Array.isArray(route.match) ? route.match : [];
      if (patterns.length === 0) continue;
      let hit = false;
      for (const p of patterns) {
        const re = this.wildcardToRegex(p);
        if (!re) continue;
        if (normalizedPaths.some((fp) => re.test(fp))) {
          hit = true;
          break;
        }
      }
      if (hit) skills.push(...add);
    }
    return this.normalizeSkillIds(skills);
  }
  ...
  buildSkillsPrompt(skillIds, mode, filePaths = []) {
    ...
    const lines = skillIds
      ...
      .join('\n');
    return [
      this.tForAI('skills_prompt_header'),
      this.tForAI('skills_prompt_mode', { mode }),
      this.tForAI('skills_prompt_must_execute_all'),
      lines,
      this.tForAI('skills_prompt_output_header'),
      this.tForAI('skills_prompt_output_rule1'),
      this.tForAI('skills_prompt_output_rule2'),
      fileHint ? `${this.tForAI('skills_prompt_target_files_header')}\n${fileHint}` : ''
    ].filter(Boolean).join('\n');
  }
  ...
  validateSkillResponse(content, skillContext) {
    ...
    const hasRequiredFields = (blockText) => {
      const hasPath = /(文件路径|File Path|Lfile)/i.test(blockText);
    ...
    return blocks.every(hasRequiredFields);
  }
  ...
  getToolsConfig() {
    ...
    return {
      ...
      maxCalls: Number(tools.maxCalls || 2),
      maxReadLines: Number(tools.maxReadLines || 400),
      maxSearchMatches: Number(tools.maxSearchMatches || 50),
      maxSearchFiles: Number(tools.maxSearchFiles || 120),
      maxListFiles: Number(tools.maxListFiles || 200),
      allow: Array.isArray(tools.allow) && tools.allow.length > 0
        ? tools.allow.map((x) => String(x).trim()).filter(Boolean)
        : ['read_file', 'get_staged_diff', 'list_files', 'search_in_file', 'get_file_outline', 'search_in_repo', 'list_changed_files']
    };
  }

  buildToolsPrompt() {
    const cfg = this.getToolsConfig();
    ...
    return [
      ...
      this.tForAI('tools_prompt_budget', { maxCalls: cfg.maxCalls }),
  ...
  extractToolCall(content) {
    ...
    const tagged = text.match(/\[TOOL_CALL\]([\s\S]*?)\[\/TOOL_CALL\]/i);
    const jsonText = tagged ? tagged[1] : text;
    const candidate = String(jsonText || '').trim();
    ...
    if (!tool) return null;
    return { tool, args: parsed.args || {} };
  }

  extractFinalReviewContent(content) {
    const text = String(content || '').trim();
    if (!text) return '';
    const match = text.match(/\[REVIEW_FINAL\]([\s\S]*?)\[\/REVIEW_FINAL\]/i);
  ...
  isNoIssueResponse(content) {
    ...
    return text === '无' || text === 'none' || text === 'no issues' || text === 'no issue';
  }

  isSubstantiveAnalysisResult(content) {
    const text = this.extractFinalReviewContent(content);
    if (!text) return false;
    if (this.extractToolCall(text)) return false;
    if (this.isNoIssueResponse(text)) return true;
    if (
      /\*\*-----代码分析结果开始-----\*\*[\s\S]*?\*\*-----代码分析结果结束-----\*\*/.test(text) ||
      /\*\*-----Git Diff代码分析结果开始-----\*\*[\s\S]*?\*\*-----Git Diff代码分析结果结束-----\*\*/.test(text) ||
      /\*\*-----Code Analysis Result Start-----\*\*[\s\S]*?\*\*-----Code Analysis Result End-----\*\*/.test(text) ||
      /\*\*-----Git Diff Code Analysis Result Start-----\*\*[\s\S]*?\*\*-----Git Diff Code Analysis Result End-----\*\*/.test(text)
    ) {
      return true;
    }
    ...
    if (hasPath && hasReason && hasSuggestion) return true;
    return /^问题\d+[:：]/m.test(text) || /^Issue\s*\d+[:：]?/mi.test(text);
  }
  ...
  async executeToolCall(call, toolsCfg) {
    ...
    if (tool === 'get_staged_diff') {
      ...
      const cmdArgs = ['-C', this.projectRoot, 'diff', '--cached', '--', ...(relPath ? [relPath] : [])];
  ...
  async resolveToolCalls(params, initialResponse, signal, toolsCfg) {
    ...
    for (let i = 0; i < maxCalls; i++) {
      ...
      const call = this.extractToolCall(content);
      if (!call) break;
      let result;
      try {
        result = await this.executeToolCall(call, toolsCfg);
      } catch (e) {
    ...
    if (this.extractToolCall(tail)) {
      ...
      messages = [...messages, { role: 'assistant', content: tail }, { role: 'user', content: finalizePrompt }];
      response = await this.createChatCompletion({ ...params, messages }, signal);
    }
  ...
  async createChatCompletion(params, signal) {
    ...
    this.appendAIRequestLog('openai_payload', params || {});
  ...
  getSystemPrompt() {
    ...
    this.systemPromptCache = [systemPrompt, outputLanguage].filter(Boolean).join('\n\n');
    return this.systemPromptCache;
  }
  ...
  getDiffSystemPrompt() {
    ...
    const outputLanguage = this.getUserOutputLanguageInstruction();
    this.diffSystemPromptCache = [diffSystemPrompt, outputLanguage].filter(Boolean).join('\n\n');
    return this.diffSystemPromptCache;
  }
  ...
  parseAIResponse(response, filePath, context = {}) {
    ...
    if (!normalizedResponse || normalizedResponse.trim() === '无') {
      ...
      if (this.isChunkedResponse(content)) {
        ...
        if (chunkInfo.isComplete) {
          ...
          collector.messages.push({ 
            role: 'user', 
            content: this.tForAI('chunk_continue_prompt') 
          });
