...
export class AIClient {
  ...
  async analyzeSmartBatch(batchData, originalBatch = null) {
    try {
      // 对于所有批次，执行常规的批量分析逻辑
      ...
      for (const file of batchData.files) {
        // 完整文件
        const prepared = await stripNoReviewForAI(file.content, file.filePath);
        const clean = await stripCommentsForAI(prepared, file.filePath);
        messages.push({
          role: 'user',
      ...
      // 使用分段响应处理
      const responseContent = await this.handleChunkedResponse(messages);
      // 批量响应：传递文件列表用于路径匹配
  ...
  async analyzeCompleteSegmentedFile(file) {
    try {
      ...
      logger.progress(`开始逐段分析文件: ${file.filePath}，共 ${file.totalChunks} 段`);
      ...
      for (let i = 0; i < file.chunks.length; i++) {
        ...
        if (this.contentCache.has(contentKey)) {
          ...
          clean = this.contentCache.get(contentKey);
        } else {
          this.cacheStats.misses++;
          const prepared = await stripNoReviewForAI(chunk.content, file.filePath);
          clean = await stripCommentsForAI(prepared, file.filePath);
          this.contentCache.set(contentKey, clean);
        }
        
        logger.info(`开始分析第 ${i + 1}/${file.totalChunks} 段 (行 ${chunk.startLine}-${chunk.endLine})，预估${chunk.tokens} tokens, 共${chunk.endLine - chunk.startLine + 1} 行代码`);
...
本段代码无明显问题`;
          ...
          if (includeStaticHints && file.staticIssues && file.staticIssues.length > 0) {
            ...
            if (segmentStaticIssues.length > 0) {
              const lines = segmentStaticIssues.map((i, idx) => `${idx + 1}. 片段（${i.risk}）：${i.message}${i.suggestion ? `；建议：${i.suggestion}` : ''}；代码片段：${i.snippet || ''}`);
...
请仅对标记为"+"的新增代码行进行审查，忽略删除行（-）和上下文行。每个问题用空行分隔，必须包含"文件路径：${fileData.filePath}"和具体的代码片段。禁止使用"第X行"等行号描述。`;
  ...
  async analyzeFilesBatch(entries) {
    try {
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        const prepared = await stripNoReviewForAI(content, filePath);
...
**大文件分段处理方式**
当我以分段形式发送内容时：
1. 分段标识：每段会以"第i/N段"标记，其中N为总分段数
2. 中间分段处理：对于第1段到第(N-1)段，你必须且只能回复"已收到第X/N段"，绝对禁止进行任何代码分析
3. 最终分段处理：只有当收到"第N/N段 - 最后一段"时，才开始合并所有分段内容进行完整的代码审查分析
4. 严格禁止：在收到最后一段之前，不得输出任何代码分析结果、问题识别或建议

**代码分析输出格式**
请严格按照以下格式返回分析结果，每个问题用空行分隔：
...
记住：你的任务是审查新增的代码变更，帮助开发者在代码合并前发现和修复潜在问题。`;
  ...
  parseIssueBlock(block, filePath, context = {}) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('文件路径：')) {
        ...
        issue.suggestion = line.replace('修改建议：', '').trim();
  ...
  // 处理分段响应的主方法
  async handleChunkedResponse(messages, requestId = null) {
    // 生成请求ID
    if (!requestId) {
    ...
    try {
      ...
      if (this.isChunkedResponse(content)) {
        ...
        if (chunkInfo.isComplete) {
          ...
          // 递归获取下一段
          return await this.handleChunkedResponse(null, requestId);
        }
      } else {
        // 不是分段响应，直接返回
        collector.chunks.push({
  ...
  // 验证AI分段确认回复
  validateChunkConfirmation(confirmText, currentChunk, totalChunks) {
    ...
    // 期望的确认格式：已收到第X/N段
    const expectedPattern = `已收到第${currentChunk}/${totalChunks}段`;
    ...
    // 检查是否包含核心确认元素
    const hasReceived = lowerText.includes('已收到') || lowerText.includes('收到') || 
                       lowerText.includes('received') || lowerText.includes('got');
    
    // 检查是否包含正确的分段信息
    const hasCorrectChunkInfo = text.includes(`第${currentChunk}/${totalChunks}段`) ||
                               text.includes(`${currentChunk}/${totalChunks}`) ||
                               (text.includes(`第${currentChunk}`) && text.includes(`${totalChunks}段`));
    
    // 检查回复长度是否合理（确认回复应该比较简短）
    const isReasonableLength = confirmText.length < AI_CONSTANTS.MAX_CONFIRMATION_TEXT_LENGTH;
    
    // 检查是否不是分析结果（分析结果通常包含这些关键词）
    const analysisKeywords = ['风险', '问题', '建议', '代码片段', '文件路径', 'risk', 'issue', 'suggestion', 'file path'];
    const isNotAnalysis = !analysisKeywords.some(keyword => lowerText.includes(keyword));
    
    // 检查是否不包含代码分析内容
    const hasNoAnalysisContent = !lowerText.includes('分析') && !lowerText.includes('analysis') &&
                                !lowerText.includes('review') && !lowerText.includes('审查');
    
    return hasReceived && hasCorrectChunkInfo && isReasonableLength && isNotAnalysis && hasNoAnalysisContent;
  }
}