...
export class CodeReviewer {
  ...
  async reviewFileDiff(fileData) {
   ...
   async applyStaticRulesToDiff(fileData, filePath) {
     ...
     // 对每个智能分段应用静态规则
      for (const segment of fileData.segments) {
       ...
       const commentRanges = this.getCachedCommentRanges(segmentContent, ext);
       
       // 计算新增行在段落中的位置
       const addedLinePositions = this.getAddedLinePositions(segment);
       ...
       for (const rule of this.rules) {
         try {
           ...
           if (typeof rule.pattern === 'function') {
             let result;
             try {
               result = rule.pattern(segmentContent);
             } catch (_) {
               result = undefined;
             }
           ...
           while ((match = regex.exec(segmentContent)) !== null) {
             ...
             // 检查匹配位置是否在新增行中
             const matchLineInSegment = this.getLineNumber(segmentContent, match.index);
             if (!addedLinePositions.has(matchLineInSegment)) {
               continue; // 不在新增行中，跳过
             }
             ...
             // 计算在原文件中的行号
             const actualLineNumber = segment.startLine + matchLineInSegment - 1;
   ...
   getAddedLinePositions(segment) {
     ...
     // 在diff模式下，我们需要检查所有包含新增代码的行
     // 由于segment.content已经是处理过的纯净代码（没有+/-前缀），
     // 而且segment.addedLineNumbers告诉我们哪些是新增行，
     // 我们简化处理：对于diff模式，检查所有非空行
  ...
  getCustomPromptForFile(filePath) {
    // 根据文件类型或路径返回自定义提示词
    // 这里可以根据需要扩展
    return '';
  }
