...
import { FixLoopRunner } from './review-fix/fix-loop-runner.js';
import { packDiffAiJobs } from './review-tools/diff-pack.js';
import { partitionSpecificFiles } from './review-tools/file-partition.js';
import { isAiConnectionError, isIncompleteReviewError, isAiFatalError } from './ai-errors.js';
...
export class CodeReviewer {
  constructor(config, rules, cancelToken = null) {
    ...
    this.issues = [];
    this.aiRan = false;
    this.aiExpected = false;
    this.reviewIncomplete = false;
    this.fixLoopStats = null;
  ...
  async reviewStagedFiles() {
    try {
      ...
      if (reviewOnlyChanges) {
        ...
        return await this.finalizeReview();
      }
    } catch (error) {
  ...
  async reviewSpecificFiles(filePaths) {
    ...
    const forceFull = this.config.ai?.forceFullFile === true;
    const reviewOnlyChanges = this.config.ai?.reviewOnlyChanges !== false;
    if (!forceFull && reviewOnlyChanges) {
      logger.info(t(this.config, 'using_diff_mode'));
      const { reviewData, touchedPaths, existingAbs } = await this.collectSpecificFilesDiff(filePaths);
      const { diffReviewData, fullPaths } = partitionSpecificFiles(
        existingAbs,
        reviewData,
        touchedPaths,
      ...
      if (diffReviewData.length > 0) {
        await this.runDiffReviewPipeline(diffReviewData, { finalize: false });
      }
      if (fullPaths.length > 0) {
        logger.info(diffReviewData.length > 0
          ? t(this.config, 'mixed_diff_fallback_full', { files: fullPaths.map((file) => path.relative(this.config.projectRoot, file)).join(', ') })
          : t(this.config, 'no_diff_fallback_full'));
        await this.reviewFilesBatchAware(fullPaths);
      } else if (diffReviewData.length === 0) {
  ...
  async reviewStagedDiff() {
    try {
      ...
      return await this.runDiffReviewPipeline(diffReviewData);
    } catch (error) {
  ...
  async collectSpecificFilesDiff(filePaths) {
    const contextMergeLines = this.config.ai?.contextMergeLines || 10;
    const diffParser = new GitDiffParser(this.config.projectRoot, contextMergeLines, this.config);
    const relativePaths = [];
    const existingAbs = [];
    ...
    for (const filePath of filePaths) {
      ...
      const fullPath = path.isAbsolute(filePath) ? filePath : path.join(this.config.projectRoot, filePath);
      if (fs.existsSync(fullPath)) {
        existingAbs.push(fullPath);
        relativePaths.push(path.relative(this.config.projectRoot, fullPath).replace(/\\/g, '/'));
      } else {
        logger.warn(t(this.config, 'file_not_exists', { file: fullPath }));
      }
    }
    ...
    if (relativePaths.length === 0) {
      return { reviewData: [], touchedPaths: [], existingAbs: [] };
    }
    const inspection = await diffParser.getFilesDiffInspection(relativePaths);
    return { ...inspection, existingAbs };
  }

  async reviewSpecificFilesDiff(filePaths) {
    try {
      if (this.isCancelled()) return this.generateResult();
      const { reviewData } = await this.collectSpecificFilesDiff(filePaths);
      return await this.runDiffReviewPipeline(reviewData);
    } catch (error) {
      logger.error(t(this.config, 'git_diff_error', { error: error?.message || String(error) }));
      throw error;
    }
  ...
  async runDiffReviewPipeline(diffReviewData, options = {}) {
    ...
    if (this.isCancelled()) {
      return finalize ? this.generateResult() : undefined;
    }
    if (this.shouldTerminateBeforeAi(hasBlockingIssues)) {
      logger.error(t(this.config, 'blocking_risk_terminate'));
      return finalize ? this.generateResult() : undefined;
    }

    if (aiEligibleFiles.length > 0 && this.aiClient) {
      const forAi = aiEligibleFiles.filter((entry) => {
        const filePath = path.resolve(this.config.projectRoot, entry.fileData.filePath);
        return this.shouldUseAI(filePath, entry.fileData.fullContent);
      });
      if (forAi.length > 0) {
        this.aiExpected = true;
      }
      this.logStaticReviewComplete();
      const packs = packDiffAiJobs(forAi, {
        maxRequestTokens: this.config.ai?.maxRequestTokens,
        maxFilesPerBatch: this.config.ai?.maxFilesPerBatch,
        tokenRatio: this.config.ai?.tokenRatio
      });
      logger.progress(t(this.config, 'ai_smart_analysis_start_progress'));
      logger.info(t(this.config, 'ai_start_n_packs', {
        files: forAi.length,
        packs: packs.length
      }));
      ...
      for (let i = 0; i < packs.length; i += concurrency) {
        ...
        await Promise.all(slice.map((pack) => this.performAIDiffPack(pack)));
      }
    }

    logger.success(t(this.config, 'git_diff_done'));
    if (finalize) return await this.finalizeReview();
  }
  ...
  async reviewFileDiff(fileData) {
    ...
    try {
      ...
      if (blockedIssues.length > 0) {
       ...
       if (this.isCancelled() || error?.isCancelled) return;
       if (isIncompleteReviewError(error) || isAiFatalError(error)) {
   ...
   async performAIDiffAnalysis(fileData, staticIssues) {
     return this.performAIDiffPack([{ fileData, staticIssues }]);
   }

   async performAIDiffPack(entries = []) {
     const jobs = (entries || []).filter((entry) => entry?.fileData);
     if (jobs.length === 0 || !this.aiClient) return;
     try {
       const aiIssues = await this.aiClient.analyzeDiffFiles(jobs);
       this.issues.push(...(aiIssues || []));
       this.aiRan = true;
       if ((aiIssues || []).length > 0) {
         logger.debug(t(this.config, 'ai_issues_found_n_dbg', { count: aiIssues.length }));
       ...
       const filePath = path.resolve(this.config.projectRoot, jobs[0].fileData.filePath);
       ...
       for (const rule of this.rules) {
         ...
         try {
           ...
           while ((match = regex.exec(addedLinesContent)) !== null) {
             ...
             if (this.isExcludedByPatterns(addedLinesContent, match.index, rule)) {
               continue;
             }
  ...
  async reviewFilesBatchAware(filePaths) {
    ...
    for (const file of filePaths) {
      ...
      try {
        ...
        if (this.aiClient && this.shouldUseAI(file, content)) {
          const contextStatic = shouldIncludeStaticHints(this.config) ? staticIssues : [];
    ...
    if (this.isCancelled()) return;
    if (this.shouldTerminateBeforeAi(anyBlocking)) {
      logger.info(t(this.config, 'blocking_risk_skip_all_ai_info'));
      return;
    }
    if (aiEligible.length > 0 && this.aiClient) {
      this.aiExpected = true;
    }
  ...
  async reviewFilesWithSmartBatching(aiEligible) {
    ...
    try {
      ...
      const smartBatching = new SmartBatching({
        maxRequestTokens: batchCfg.maxRequestTokens,
        minFilesPerBatch: batchCfg.minFilesPerBatch,
        maxFilesPerBatch: batchCfg.maxFilesPerBatch,
        tokenRatio: batchCfg.tokenRatio,
        chunkOverlapLines: batchCfg.chunkOverlapLines
      });
      ...
      if (this.isCancelled() || error?.isCancelled) throw error;
      if (isIncompleteReviewError(error) || isAiFatalError(error)) {
        this.handleAiFailure(error);
        this.aiRan = true;
        return;
      ...
      try {
        const max = Number(batchCfg.maxFilesPerBatch);
        const step = Number.isFinite(max) && max > 0 ? max : (aiEligible.length || 1);
        const batches = [];
        for (let i = 0; i < aiEligible.length; i += step) {
          batches.push(aiEligible.slice(i, i + step));
        }
        ...
        if (this.isCancelled() || fallbackError?.isCancelled) throw fallbackError;
        if (isIncompleteReviewError(fallbackError) || isAiFatalError(fallbackError)) {
          this.handleAiFailure(fallbackError);
          this.aiRan = true;
          return;
        }
  ...
  async processBatchesConcurrently(batches, smartBatching) {
    ...
    try {
      const { issues: allIssues } = await this.aiClientPool.executeConcurrentBatches(batches, progressCallback);
      this.issues.push(...allIssues);

      if (!this.isCancelled()) {
        logger.success(t(this.config, 'ai_analysis_done_summary', {
          totalIssues: allIssues.length,
          elapsed: ((Date.now() - progressTracker.startTime) / 1000).toFixed(1)
        }));
      }
    } catch (error) {
      if (error?.isCancelled) throw error;
      if (isIncompleteReviewError(error) || isAiFatalError(error)) throw error;
  ...
  async processBatchesSerially(batches, smartBatching) {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      if (batch.isLargeFileSegment) {
        ...
        const fileList = batch.items.map(item => item.originalFilePath || item.filePath).join(',');
        logger.info(t(this.config, 'serial_small_file_batch_info', { index: i + 1, total: batches.length, files: fileList }));
      }

      const result = await this.aiClient.analyzeSmartBatch(formattedBatch, batch);
      const aiIssues = Array.isArray(result) ? result : (result?.issues || []);
      this.issues.push(...aiIssues);
      if (this.isCancelled()) break;
      logger.success(t(this.config, 'batch_done_success', { index: i + 1, total: batches.length }));
    }
    if (!this.isCancelled()) {
      logger.success(t(this.config, 'ai_analysis_done_summary', {
        totalIssues: this.issues.length - issuesBefore,
    ...
    for (const rule of this.rules) {
      ...
      try {
        ...
        while ((match = regex.exec(content)) !== null) {
          ...
          if (this.isExcludedByPatterns(content, match.index, rule)) {
            continue;
          }
  ...
  isExcludedByPatterns(content, matchIndex, rule) {
    const excludePatterns = Array.isArray(rule.excludePatterns) ? rule.excludePatterns : null;
    ...
    for (const ex of excludePatterns) {
      try {
        const rx = this.getCachedRegex(ex, 'i');
        if (rx.test(line)) return true;
      } catch (_) {
        // ignore invalid exclude pattern
      }
    }
    return false;
  }
  ...
  shouldUseAI(filePath, content) {
    ...
    const maxFileSizeKB = Number(this.config.ai?.maxFileSizeKB);
    if (Number.isFinite(maxFileSizeKB) && maxFileSizeKB > 0 && content.length > maxFileSizeKB * 1024) {
  ...
  shouldTerminateBeforeAi(anyBlocking) {
    if (!anyBlocking) return false;
    return this.config.ai?.runWhenBlocked !== true;
  }

  markReviewIncomplete(error) {
    this.reviewIncomplete = true;
    const msg = error?.message || t(this.config, 'ai_review_incomplete');
    logger.error(msg);
  }

  handleAiFailure(error) {
    if (this.isCancelled() || error?.isCancelled) return;
    this.markReviewIncomplete(error);
  }

  async finalizeReview() {
    if (this.aiRan && this.aiClient) {
      const runner = new FixLoopRunner({ reviewer: this, config: this.config });
      this.fixLoopStats = await runner.run();
    }
    return this.generateResult();
  }

  removeIssuesForFiles(filePaths, source = null) {
    const normalized = new Set(
      filePaths.map((file) => path.resolve(this.config.projectRoot, file))
    );
    this.issues = this.issues.filter((issue) => {
      const issueFile = path.resolve(this.config.projectRoot, issue.file);
      if (!normalized.has(issueFile)) return true;
      if (source && issue.source !== source) return true;
      return false;
    });
  }

  async reReviewFilesAfterFix(filePaths) {
    if (!filePaths || filePaths.length === 0 || !this.aiClient) return;
    const absPaths = filePaths.map((file) => path.resolve(this.config.projectRoot, file));
    this.removeIssuesForFiles(absPaths);

    const reviewOnlyChanges = this.config.ai?.reviewOnlyChanges === true;
    if (reviewOnlyChanges) {
      const contextMergeLines = this.config.ai?.contextMergeLines || 10;
      const diffParser = new GitDiffParser(this.config.projectRoot, contextMergeLines, this.config);
      const diffReviewData = await diffParser.getStagedDiffReviewData();
      const targets = diffReviewData.filter((entry) => {
        const abs = path.resolve(this.config.projectRoot, entry.filePath);
        return absPaths.includes(abs);
      });
      for (const fileData of targets) {
        if (this.isCancelled()) break;
        const filePath = path.resolve(this.config.projectRoot, fileData.filePath);
        const staticIssues = await this.applyStaticRulesToDiff(fileData, filePath);
        this.issues.push(...staticIssues);
        await this.performAIDiffAnalysis(fileData, staticIssues);
      }
      return;
    }
    ...
    for (const file of absPaths) {
      if (this.isCancelled()) break;
      if (!this.isReviewableFile(file)) continue;
      const content = await this.getFileContent(file);
      if (!content) continue;
      const disable = this.computeDisableRanges(content, file);
      const staticIssues = this.applyStaticRules(content, file, disable);
      this.issues.push(...staticIssues);
      if (this.shouldUseAI(file, content)) {
        const contextStatic = shouldIncludeStaticHints(this.config) ? staticIssues : [];
        aiEligible.push({ filePath: file, content, staticIssues: contextStatic });
      }
    }
  ...
  logStaticReviewComplete() {
    const staticIssues = (this.issues || []).filter((issue) => issue?.source === 'static');
    const riskLevels = this.config.riskLevels || {};
    const blockingCount = staticIssues.filter((issue) => {
      const levelCfg = riskLevels[issue.risk || 'suggestion'];
      return levelCfg && levelCfg.block === true;
    }).length;
    if (blockingCount > 0) {
      logger.progress(t(this.config, 'static_review_done_blocking', {
        count: staticIssues.length,
        blocking: blockingCount
      }));
      return;
    }
    logger.progress(t(this.config, 'static_review_done_ok', {
      count: staticIssues.length
    }));
  }
  ...
  generateResult() {
    ...
    if (this.aiExpected && !this.aiRan && !this.reviewIncomplete && this.aiClient) {
      this.reviewIncomplete = true;
      logger.error(t(this.config, 'ai_review_incomplete'));
    }
    ...
    return {
      ...
      aiRan: !!this.aiRan,
      reviewIncomplete: !!this.reviewIncomplete,
      fixLoop: this.fixLoopStats,
      summary: {
        total: deduped.length,
        blocking: deduped.filter(issue => blockingLevels.includes(issue.risk)).length,
        fixesApplied: this.fixLoopStats?.applied || 0
      }
    };
  }
