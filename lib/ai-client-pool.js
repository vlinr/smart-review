...
import { isAiFatalError } from './ai-errors.js';
...
export class AIClientPool {
  ...
  initializePool() {
    ...
    for (let i = 0; i < this.poolSize; i++) {
      ...
      const cancelToken = this.config?.cancelToken;
  ...
  async getAvailableClient() {
    ...
    return new Promise((resolve) => {
      const entry = {
        resolve: (client) => {
          if (entry.off) {
            try { entry.off(); } catch (e) {}
          }
          resolve(client);
        },
        off: null
      };
      const onCancel = this.config?.cancelToken?.onCancel;
  ...
  async executeConcurrentBatches(batches, progressCallback = null) {
    if (this.isCancelled()) return { issues: [], totalDurationMs: 0 };
    if (!batches || batches.length === 0) {
      return { issues: [], totalDurationMs: 0 };
    }
    ...
    try {
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          if (isAiFatalError(result.reason) && !fatalError) {
  ...
  async processBatchWithRetry(batch, batchIndex, progressCallback = null, retryCount = 0) {
    ...
    try {
      if (this.isCancelled()) return { issues: [], durationMs: 0 };
      // 获取可用客户端
      const client = await this.getAvailableClient();
      if (!client) return { issues: [], durationMs: 0 };
      ...
      if (this.isCancelled() || /abort|cancel|中断/i.test(msg) || error?.isCancelled) {
        return { issues: [], durationMs: 0 };
      }
      if (isAiFatalError(error)) {
        throw error;
      }
  ...
  isCancelled() {
    return this.config?.cancelToken && typeof this.config.cancelToken.isCancelled === 'function' && this.config.cancelToken.isCancelled();
  }
  ...
  cleanup() {
    ...
    this.waitingQueue.forEach(entry => {
      entry.resolve(null); // 返回null表示池已关闭
    });
    this.waitingQueue = [];
