    ...
    for (const file of files) {
      ...
      if (originalTokens > this.maxFileTokens) {
        ...
        chunks.forEach(chunk => {
          ...
          item.needsWaiting = true;
          item.segmentGroup = filePath;

          const batch = this.getBatchFromPool();
          batch.batchIndex = -1; // 稍后重新编号
          batch.items = [item]; // 每个批次只包含一个分段
          batch.totalTokens = chunk.tokens;
          batch.fileCount = 1;
          batch.utilization = (chunk.tokens / this.config.maxRequestTokens) * 100;
          batch.isLargeFileSegment = true;
          batch.segmentedFile = filePath;
          batch.totalSegments = chunks.length;
          batch.currentSegment = chunk.chunkIndex + 1; // 当前段号（1-based）
    ...
    largeFiles.forEach(file => {
      ...
      segments.forEach(segment => {
        const batch = {
          ...
          fileCount: 1,
          utilization: (segment.tokens / this.config.maxRequestTokens) * 100,
          isSegmented: true
        };
        batches.push(batch);
      });
