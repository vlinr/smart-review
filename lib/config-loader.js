...
const MERGE_UNION_ARRAY_KEYS = new Set(['ignoreFiles']);
...
export class ConfigLoader {
  ...
  async loadConfig() {
    ...
    // 用户配置覆盖内部默认；未写的字段走内部默认，不要求写进 smart-review.json
  ...
  deepMerge(target, source) {
    ...
    for (const key in source) {
      ...
      if (incoming && typeof incoming === 'object' && !Array.isArray(incoming)) {
        result[key] = this.deepMerge(result[key] || {}, incoming);
        continue;
      }

      if (Array.isArray(incoming) && MERGE_UNION_ARRAY_KEYS.has(key) && Array.isArray(result[key])) {
        result[key] = uniqueStrings([...result[key], ...incoming]);
        continue;
      }
